    ...
    def drop_pairs(self):
        ...
        for i in range(len(cards) - 1):
            if cards[i] and cards[i].rank == cards[i + 1].rank:
                drop_cards += [cards[i], cards[i + 1]]
                cards[i] = cards[i + 1] = None
        self.cards = [card for card in self.cards if card not in drop_cards]
        return drop_cards
...
class AutoOldmaid:
    ...
    def _drop_pairs(self):
        print("------ Drop Pairs ------")
        for player in self.players:
            drop_cards = player.drop_pairs()
            if drop_cards:
                print(f"{player.name} dropped {drop_cards}")
    ...
    def _turn(self):
        ...
        for i, player in enumerate(players):
            ...
            for next_player in players[i+1:] + players[:i]:
                ...
                return
            pull_card, drop_cards = player.play(next_player)
            print(f"{player.name} pulled {pull_card} from {next_player.name}")
            if next_player.win():
